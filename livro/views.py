...
def ver_livros(request, id):
    if request.session.get('usuario'):
        ...
        if request.session.get('usuario') == livro.usuario.id:
            ...
            livros = Livros.objects.filter(usuario = usuario)#coloquei
            total_livros = livros.count()#coloquei
            form = CadastroLivro()
            form.fields['usuario'].initial = request.session['usuario']
            form.fields['categoria'].queryset=Categoria.objects.filter(usuario = usuario)
            ...
            return HttpResponse('Este livro não lhe pertence !!')
    return redirect('/auth/login/?status=2')
...
def cadastrar_categoria(request):
    ...
    if int(id_usuario) == int(request.session.get('usuario')):
        ...
        return HttpResponse('Pare de tentar sabotar o sistema isso não é correto!')
